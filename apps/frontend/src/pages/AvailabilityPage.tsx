...
export default function AvailabilityPage() {
  ...
  const loadBookings = async () => {
    try {
      ...
      const filtered = allBookings.filter((b: Booking) => 
        b.rentable_item_id === selectedItem?.id
      );
  ...
  const handleCreateHold = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Đã tạo hold thành công! Hold sẽ tự động hết hạn sau thời gian quy định.');
      ...
      alert(error.response?.data?.message || 'Không thể tạo hold');
    }
  };
  ...
  const handleConfirmBooking = async (id: string) => {
    ...
    try {
      ...
      alert(error.response?.data?.message || 'Không thể xác nhận booking');
    }
  };
  ...
  return (
    <Layout userRole="LANDLORD">
      <div className="p-8">
        {/* Header */}
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-gray-900">Lịch tình trạng & Booking</h1>
          <p className="text-gray-600 mt-1">Quản lý lịch available, hold và booking</p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow p-4 mb-6">
          <div className="flex justify-between items-center">
            ...
            <div className="flex gap-3">
              <button
                onClick={() => setShowCreateHoldModal(true)}
                    ...
                    <div className="space-y-1">
                      {dayBookings.map(booking => (
                        <div
                          ...
                          title={`${translateStatus(booking.status)} - ${booking.quantity} slot`}
                        >
                    ...
                    <div className="space-y-1">
                      {dayBookings.slice(0, 2).map(booking => (
                        <div
                          ...
                          className={`text-xs p-1 rounded border ${getStatusColor(booking.status)}`}
                        >
                          {translateStatus(booking.status).slice(0, 6)}
                        </div>
                      ))}
        ...
        <div className="bg-white rounded-lg shadow p-4 mb-6">
          ...
          <div className="flex flex-wrap gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-gray-700">Đã xác nhận</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-4 h-4 bg-blue-100 border border-blue-300 rounded"></div>
              <span className="text-sm text-gray-700">Đang giữ chỗ</span>
            </div>
            <div className="flex items-center gap-2">
          ...
          <div className="overflow-x-auto">
            {bookings.length === 0 ? (
              <div className="p-12 text-center text-gray-500">
                ...
                <p>Chưa có booking nào cho item này</p>
              </div>
            ) : (
        ...
        {showCreateHoldModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-8 max-w-md w-full">
              <h2 className="text-2xl font-bold text-gray-900 mb-4">Tạo Hold (Giữ chỗ tạm)</h2>
              <p className="text-sm text-gray-600 mb-4">
                Hold sẽ tự động hết hạn sau thời gian quy định. Dùng để giữ chỗ tạm thời cho khách.
              </p>
              <form onSubmit={handleCreateHold} className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    type="datetime-local"
                    value={holdForm.start_time}
                    onChange={(e) => setHoldForm({ ...holdForm, start_time: e.target.value })}
                ...
                <div>
                  ...
                  <input
                    type="datetime-local"
                    value={holdForm.end_time}
                    onChange={(e) => setHoldForm({ ...holdForm, end_time: e.target.value })}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg"
                    required
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setHoldForm({ ...holdForm, quantity: parseInt(e.target.value) })}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg"
                  />
                </div>
                <div>
                ...
                <div className="flex gap-3 mt-6">
                  <button
                    type="button"
                    onClick={() => setShowCreateHoldModal(false)}
        ...
        {showCreateBookingModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-8 max-w-md w-full max-h-[90vh] overflow-y-auto">
              <h2 className="text-2xl font-bold text-gray-900 mb-4">Tạo Booking mới</h2>
              <form onSubmit={handleCreateBooking} className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    Tenant Party ID (tùy chọn)
                  </label>
                  <input
                    type="text"
                    value={bookingForm.tenant_party_id}
                    onChange={(e) => setBookingForm({ ...bookingForm, tenant_party_id: e.target.value })}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg"
                    placeholder="ID của tenant"
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
